...
* Clients hold an identity link wrapper key (only known to clients)
* Used to encrypt and decrypt identity link keys, which are in turn used to encrypt and decrypt identity links
* The client creating the group samples the identity link wrapper key freshly upon group creation
* Clients joining the group receive the key encrypted via a group info extension (i.e. either via a Welcome or via an External Init package)
* The key is fixed and is never rotated

## User profile key encryption

* Generated freshly by the group creator and distributed to group members alongside the identity link wrapper key
